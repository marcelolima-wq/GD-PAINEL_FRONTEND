...
function playTv(isRetry=false){
  ...
  if(media.type==='video'){image.hidden=true;video.hidden=false;video.src=media.src;video.muted=true;video.currentTime=0;video.onerror=()=>{video.onerror=null;if(media.source==='blob'){advanceTv();return}if(tvVideoRetryCount<2){tvVideoRetryCount++;tvVideoRetryTimer=setTimeout(()=>playTv(true),3000)}else{tvVideoRetryCount=0;advanceTv()}};video.play().catch(()=>{});video.onended=advanceTv;video.ontimeupdate=()=>{if(video.duration)$('tvProgress').style.width=`${video.currentTime/video.duration*100}%`}}
  else{video.hidden=true;const probe=new Image();probe.onload=()=>{if(mediaToken!==tvMediaToken)return;tvVideoRetryCount=0;image.style.backgroundImage=`url("${media.src}")`;image.hidden=false;const duration=Math.max(1,Number(item.duration)||8)*1000,start=performance.now();const tick=now=>{if(mediaToken!==tvMediaToken)return;const progress=Math.min(1,(now-start)/duration);$('tvProgress').style.width=`${progress*100}%`;if(progress<1)tvFrame=requestAnimationFrame(tick);else advanceTv()};tvFrame=requestAnimationFrame(tick)};probe.onerror=()=>{if(mediaToken!==tvMediaToken)return;if(media.source==='blob'){advanceTv();return}if(tvVideoRetryCount<2){tvVideoRetryCount++;tvVideoRetryTimer=setTimeout(()=>playTv(true),3000)}else{tvVideoRetryCount=0;advanceTv()}};probe.src=media.src}
}
